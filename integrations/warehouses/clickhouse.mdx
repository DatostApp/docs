...
<Warning>
  Large result sets are capped. Queries run with `max_result_rows` (default `1000`) and `result_overflow_mode = 'break'`, so ClickHouse stops streaming once the cap is hit and the response is marked as truncated. Schema-listing queries use a higher internal cap.
</Warning>
