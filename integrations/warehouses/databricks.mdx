...
<AccordionGroup>
  ...
  <Accordion title="Row limits and truncation">
    Statements run with `format: JSON_ARRAY` and `disposition: INLINE`. Query results are capped (default 1,000 rows) and flagged as `truncated` when the cap is hit. Schema introspection uses higher caps internally.
  </Accordion>
