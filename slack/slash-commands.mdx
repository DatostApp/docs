...
Datost's internal team also registers `/datost-dev`, which repoints the dev Slack app's event subscription URL to a given preview backend (for example, a Railway PR preview). It's gated behind the dev app's signing secret and is not available to customer workspaces — external users can safely ignore it.
